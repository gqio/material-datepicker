export default class MaterialDatepicker extends HTMLElement {
  ...
  showMonthView() {
    ...
    this.monthView = this.shadowRoot
      .querySelector("#month-view")
    ...
    this.monthViewContainer = this.monthView = this.shadowRoot.querySelector(
      "#month-view-container"
    );
    this.daysContainer = this.shadowRoot.querySelector("#days-container");
    this.host = this.daysContainer.getRootNode().host;
    this.currentMonthContainer = this.shadowRoot.querySelector(
      "#current-month-container"
    );
  ...
  showYearsView() {
    ...
    this.yearsView = this.shadowRoot
      .querySelector("#years-view")
      .content.cloneNode(true);
    ...
    this.yearsViewContainer = this.shadowRoot.querySelector(
      "#years-view-container"
    );
    ...
    this.yearsViewContainer.scrollTop =
      offset -
      this.yearsViewContainer.offsetTop -
      this.yearsViewContainer.offsetHeight / 2 +
      currentYear.offsetHeight / 2;

    this.mainHeader.classList.add("years-view");
    this.mainHeader.classList.remove("month-view");

    this.yearsViewContainer.addEventListener(
      "click",
      this.handleYearClick.bind(this)
    );
  }
  ...
  displayMonth(date) {
    ...
    this.currentMonthContainer.textContent = `${this.getMonthName(
      this.currentDate
    )} ${this.currentYear}`;
  ...
  getShortDays() {
    ...
    for (let i = 0; i <= endDay; i++) {
      ...
      const narrow = new Intl.DateTimeFormat(this.locale, {
        weekday: "narrow"
      }).format(date);
      const short = new Intl.DateTimeFormat(this.locale, {
        weekday: "short"
      }).format(date);
